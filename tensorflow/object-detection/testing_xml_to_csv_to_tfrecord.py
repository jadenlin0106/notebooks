...
def xml_to_csv(path):
    ...
    for file in files:
        for xml_file in glob.glob(path + '/' + file + '/*.xml'):
                jpg_file_path = xml_file.split('/')
                jpg_file = jpg_file_path[3].split('.xml')
                tree = ET.parse(xml_file)
                root = tree.getroot()
                for member in root.findall('object'):
                        value = ('JPEGImages/All/'+jpg_file_path[2]+'/'+jpg_file[0]+'.jpg',
                                '1920',         #int(root.find('size')[0].text),
                                '1080',         #int(root.find('size')[1].text),
                                member[0].text,
                                int(member[1][0].text),
                                int(member[1][1].text),
                                int(member[1][2].text),
